...
// Импортируем критерии из основного файла
...
export async function GET(
  ...
  const criteriaSet = criteriaSets.find(
    (cs) => cs.objectTypes.includes(type as any) || cs.id === type
  );

  if (!criteriaSet) {
    return NextResponse.json(
      { error: 'Набор критериев не найден' },
      { status: 404 }
    );
  }

  return NextResponse.json({ data: criteriaSet });
}
